...
def login():
 # connect
    ...
    # esscribe los mensajes de error si no pasa la validacion...
    msg = ''
    # verifica si "usuario" y "contraseña" cuando se manda la peticion post existe
    if request.method == 'POST' and 'Usuario' in request.form and 'password' in request.form:
        # crea variables para acceder facilmente
        Usuario = request.form['Usuario']
        password = request.form['password']
        # verifica que la cuenta exita en Mysql
        cursor.execute(
    ...
    # If account exists in accounts table in out database
        if account:
            # crea los datos de la session, que nos permitira el acceso a las demas rutas
...
def register():
 # connect
  ...
  # Check if account exists using MySQL
        ...
        cursor.execute(
            'SELECT * FROM usuarios WHERE Estado=1 AND Usuario =%s OR Correo = %s', (Usuario, Correo))
        account = cursor.fetchone()

        # If account exists show error and validation checks
        if account:
            msg = 'El usuario ya existe o correo ya existe'
        ...
        msg = 'Por favor debe llenar el formulario!'
...
def add_Modulo():
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        flash('Modulo agregado correctamente')
...
def add_Disponibilidad():
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        flash('Disponibilidad agregada correctamente')
...
def Agregar_Solicitudes():
 # Check if account exists using MySQL
    ...
    if 'loggedin' in session:
        ...
        if session['ID_ROL'] == 1:
            cursor1.execute(
                'SELECT Rl.ID_RELACION, CONCAT(ase.Nombres, " ", ase.Apellidos) AS Asesor ,Asg.Asignatura,Rl.Hora_Inicial,Rl.Hora_Final,ase.Telefono,Rl.Estado_Solicitud  FROM relacion_usuario Rl INNER JOIN asignatura Asg ON (Asg.ID_ASIGNATURA=Rl.ID_ASIGNATURA) INNER JOIN usuarios ase ON (ase.ID_USUARIO=Rl.ID_ASESOR) INNER JOIN usuarios est ON (est.ID_USUARIO=Rl.ID_USUARIO)')
            data2 = cursor1.fetchall()
        if session['ID_ROL'] == 2:
            cursor1.execute(
                'SELECT Rl.ID_RELACION, CONCAT(ase.Nombres, " ", ase.Apellidos) AS Asesor, CONCAT(est.Nombres, " ", est.Apellidos) AS Estudiante ,Asg.Asignatura,Rl.Hora_Inicial,Rl.Hora_Final,ase.Telefono,Rl.Estado_Solicitud  FROM relacion_usuario Rl INNER JOIN asignatura Asg ON (Asg.ID_ASIGNATURA=Rl.ID_ASIGNATURA) INNER JOIN usuarios ase ON (ase.ID_USUARIO=Rl.ID_ASESOR) INNER JOIN usuarios est ON (est.ID_USUARIO=Rl.ID_USUARIO) WHERE Rl.Estado=1')
            data2 = cursor1.fetchall()
        if session['ID_ROL'] == 3:
            cursor1.execute(
                'SELECT Rl.ID_RELACION, CONCAT(ase.Nombres, " ", ase.Apellidos) AS Asesor ,Asg.Asignatura,Rl.Hora_Inicial,Rl.Hora_Final,ase.Telefono,Rl.Estado_Solicitud FROM relacion_usuario Rl INNER JOIN asignatura Asg ON (Asg.ID_ASIGNATURA=Rl.ID_ASIGNATURA) INNER JOIN usuarios ase ON (ase.ID_USUARIO=Rl.ID_ASESOR) INNER JOIN usuarios est ON (est.ID_USUARIO=Rl.ID_USUARIO) WHERE Rl.Estado=1 AND Rl.ID_USUARIO=%s', (session['id']))
            data2 = cursor1.fetchall()
        cursor.close()
...
def Asignar_Solicitud():
    ...
    cursor1.execute('INSERT INTO relacion_usuario VALUES (NULL,%s, %s, %s, %s, %s, %s,%s)',
                    (session['id'], select, ID_Asesor, 1, Hora_Inicial, Hora_Final, "PENDIENTE"))
    ...
    flash('Su solicitud a sido asignada')
    return redirect(url_for('Agregar_Solicitudes'))


@app.route('/update_Solicitud_edit', methods=['POST'])
def update_Solicitud_edit():
    if request.method == 'POST':
        ID_Asignacion = request.form.get('ID_Asignacion')
        print(ID_Asignacion)
        conn = mysql.connect()
        cur = conn.cursor(pymysql.cursors.DictCursor)
        cur.execute("""UPDATE relacion_usuario SET Estado=%s ,Estado_Solicitud = %s WHERE ID_RELACION=%s""",
                    (2, "ATENDIDA", ID_Asignacion))
        flash('Solicitud atentida correctamente')
        conn.commit()
        return redirect(url_for('Agregar_Solicitudes'))


@app.route('/update_Solicitud_delete', methods=['POST'])
def update_Solicitud_delete():
    if request.method == 'POST':
        ID_Asignacion = request.form.get('ID_Asignacion')
        print(ID_Asignacion)
        conn = mysql.connect()
        cur = conn.cursor(pymysql.cursors.DictCursor)
        cur.execute("""UPDATE relacion_usuario SET Estado = %s, Estado_Solicitud=%s WHERE ID_RELACION=%s""",
                    (2, "CANCELADA", ID_Asignacion))

        """ cursor2.execute('UPDATE disponibilidad_asesor SET Estado = %s, Estado_Solicitud=%s  WHERE ID=%s',
                    (2, "Cancelada", ID_Asignacion)) """

        flash('Solicitud cancelada correctamente')
        conn.commit()
        return redirect(url_for('Agregar_Solicitudes'))
